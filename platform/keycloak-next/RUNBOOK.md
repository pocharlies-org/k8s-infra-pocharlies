...
## 15. Realm drift sweep (`keycloak-role-drift`, INFRA-250)

The auditor secret is the 1Password item `keycloak-rbac-auditor` (vault
`k8s-pocharlies`, field `password`), created headless with the ESO service
account and a value generated by `op` (never on argv, never printed). To
rotate it, regenerate the field and let the next sync push it:

```bash
# on the x86, token of the ESO 1Password service account in the environment only
OP_SERVICE_ACCOUNT_TOKEN="$(kubectl -n external-secrets-operator get secret \
  onepassword-service-account -o jsonpath='{.data.token}' | base64 -d)" \
  op item edit keycloak-rbac-auditor --vault k8s-pocharlies \
  --generate-password=40,letters,digits >/dev/null
kubectl -n keycloak annotate externalsecret keycloak-rbac-auditor \
  force-sync="$(date +%s)" --overwrite
# then an ArgoCD sync of k8s-infra re-runs the PostSync, which sets the new secret
```

Reconciler evidence after a sync (C-4 of the INFRA-219 architecture is the
two `probe` lines):

```bash
kubectl -n keycloak wait --for=condition=complete job/keycloak-rbac-auditor-client --timeout=900s
kubectl -n keycloak logs job/keycloak-rbac-auditor-client
# {"client_id":"keycloak-rbac-auditor","probe":"read-client-secret","denied":true,"status":"403"}
# {"client_id":"keycloak-rbac-auditor","probe":"post-role-mapping","denied":true,"status":"403"}
# {"client_id":"keycloak-rbac-auditor","management_roles":"query-groups,query-users,view-realm,view-users","present":true,"exact":true}
```

Run the sweep now instead of waiting for the next quarter hour:
...
A red run prints one `DRIFT:` line per finding (exit 1) — e.g.
`DRIFT: qa-con-rol@e-dani.com tiene agentgateway-read:gsc no declarada` —
