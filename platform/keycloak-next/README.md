...
- `keycloak-rbac-auditor` (Password item, INFRA-250)
  - `password` — the client secret of `keycloak-rbac-auditor`, generated by
    `op` at creation (`--generate-password`); nobody types or prints it
...
catalog change is what the next run checks. A failed Job raises the existing
`K8sCronJobFailed` alert (Telegram, topic Crons); there is no other alerting.

It authenticates as the dedicated client `keycloak-rbac-auditor`
(confidential, service account only, `fullScopeAllowed=false`) whose service
account holds exactly the `realm-management` roles `view-realm`, `view-users`,
`query-users`, `query-groups` — no `view-clients` (it would read client
secrets), no `manage-*`. The PostSync `keycloak-rbac-auditor-client.yaml`
(`scripts/keycloak-rbac-auditor-client.sh`, kcadm, sync-wave 22) sets the
client and its secret from the ExternalSecret, grants and scopes those four,
and then fails closed on any extra role — direct, effective, in the client
scope, of another client, of the realm, by group, or in the minted token —
and on either live denial not holding: with the auditor's own token,
`GET clients/{uuid}/client-secret` and a `POST users/{id}/role-mappings/realm`
(empty body) must be refused. It never removes anything.

The CronJob does not mount `keycloak-bootstrap` nor `keycloak-automation`,
...
Operations: `RUNBOOK.md` section 14.
