...
## Longhorn CODEX_HOME encryption key

1. In a trusted shell with history and tracing disabled, generate a high-entropy
   passphrase and write it directly to Vault path
   `secret/longhorn/openclaw-codex-crypto`, property `CRYPTO_KEY_VALUE`. Never
   print it or pass it as a command-line argument.

2. Back up Vault and prove the key can be recovered before creating the PVC.
   Loss of this key makes the encrypted CODEX_HOME volume and its backups
   unrecoverable.

3. Reconcile `externalsecret/openclaw-codex-crypto` in `longhorn-system`. Check
   only its Ready condition and key names; never decode the Secret during normal
   validation.

4. Confirm `cryptsetup` is installed and `dm_crypt` is loaded on all three KS5
   Longhorn nodes. Create the CODEX_HOME PVC with StorageClass
   `longhorn-openclaw-codex-encrypted`, then verify the Longhorn volume reports
   `encrypted=true`, three healthy replicas, node selector `ks5-nvme`, and disk
   selector `nvme`.

5. Perform a write/read checksum test, detach/reattach test, and backup/restore
   test before placing the Codex credentials or home data on the volume.

Do not rotate the Vault property as if it were a normal API token. ESO uses
`CreatedOnce` deliberately. Encryption-key changes require a separate LUKS key
migration runbook and a verified rollback path.
